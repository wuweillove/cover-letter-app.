...
with tab1:
    ...
    with st.expander(get_text('create_step2_title', lang), expanded=current_step==2):
        ...
        if resume_input and job_input:
            ...
            st.info(get_text('create_step2_complete_msg', lang))
    ...
    with st.expander(get_text('create_step3_title', lang), expanded=current_step==3):
        ...
        emphasis_areas = st.multiselect(
            ...
            [
                ...
                get_text('emphasis_problem', lang),
                get_text('emphasis_innovation', lang),
                get_text('emphasis_team', lang),
                get_text('emphasis_project', lang),
    ...
    with st.expander(get_text('create_step4_title', lang), expanded=current_step==4):
        ...
        with col2:
            include_analysis = st.checkbox(
                get_text('create_step4_analysis', lang),
                value=True,
                help=get_text('create_step4_analysis_help', lang)
            )
        ...
        if st.session_state.generated_versions:
            ...
            if len(st.session_state.generated_versions) > 1:
                ...
                for idx, (col, version) in enumerate(zip(cols, st.session_state.generated_versions)):
                    with col:
                        st.markdown(f"**{get_text('create_step5_version', lang).format(version['version'])}**")
                        st.text_area(
            ...
            # Export options
            st.markdown(f"### {get_text('create_step5_export_title', lang)}")
            ...
            with col2:
                if st.button(get_text('btn_download_word', lang), use_container_width=True):
                    st.info(get_text('create_step5_word_coming', lang))
            
            with col3:
                if st.button(get_text('btn_copy_clipboard', lang), use_container_width=True):
                    st.success(get_text('create_step5_copy_msg', lang))
            ...
            st.info(get_text('create_step5_generate_first', lang))
...
with tab2:
    ...
    if not st.session_state.generated_versions:
        st.info(get_text('analysis_generate_first', lang))
    else:
        ...
        if hasattr(st.session_state, 'job_data'):
            ...
            with score_cols[0]:
                ...
                <div class="score-card">
                    ...
                    <div class="score-label">{get_text('analysis_overall_score', lang)}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with score_cols[1]:
                ...
                <div class="score-card">
                    ...
                    <div class="score-label">{get_text('analysis_ats_score', lang)}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with score_cols[2]:
                ...
                <div class="score-card">
                    ...
                    <div class="score-label">{get_text('analysis_grammar_score', lang)}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with score_cols[3]:
                ...
                <div class="score-card">
                    ...
                    <div class="score-label">{get_text('analysis_keywords_score', lang)}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with score_cols[4]:
                if skills_match:
                    ...
                    <div class="score-card">
                        ...
                        <div class="score-label">{get_text('analysis_skills_score', lang)}</div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            with col1:
                ...
                with st.expander(get_text('analysis_ats_title', lang), expanded=True):
                    st.markdown(get_text('analysis_score_label', lang).format(ats_score['score']))
                    st.progress(ats_score['score'] / 100)
                    
                    st.markdown(get_text('analysis_strengths', lang))
                    for strength in ats_score.get('strengths', []):
                        st.success(f"• {strength}")
                    
                    st.markdown(get_text('analysis_improvements', lang))
                    for improvement in ats_score.get('improvements', []):
                        st.warning(f"• {improvement}")
                
                # Grammar Check
                with st.expander(get_text('analysis_grammar_title', lang)):
                    st.markdown(get_text('analysis_score_label', lang).format(grammar_results['score']))
                    st.progress(grammar_results['score'] / 100)
                    
                    if grammar_results.get('issues'):
                        st.markdown(get_text('analysis_issues_found', lang))
            ...
            with col2:
                # Keyword Analysis
                with st.expander(get_text('analysis_keywords_title', lang), expanded=True):
                    st.markdown(get_text('analysis_coverage', lang).format(keyword_analysis['coverage']))
                    st.progress(keyword_analysis['coverage'] / 100)
                    
                    st.markdown(get_text('analysis_matched_keywords', lang))
                    ...
                    st.markdown(get_text('analysis_missing_keywords', lang))
                ...
                if skills_match:
                    with st.expander(get_text('analysis_skills_title', lang)):
                        st.markdown(get_text('analysis_match', lang).format(skills_match['match_percentage']))
                        st.progress(skills_match['match_percentage'] / 100)
                        
                        st.markdown(get_text('analysis_highlighted_skills', lang))
                        for skill in skills_match.get('matched_skills', []):
                            st.success(f"• {skill}")
                        
                        st.markdown(get_text('analysis_consider_adding', lang))
            ...
            for i, suggestion in enumerate(suggestions, 1):
                with st.expander(get_text('analysis_suggestion', lang).format(i, suggestion['title'])):
                    ...
                    if st.button(get_text('btn_apply_suggestion', lang).format(i), key=f"apply_sug_{i}"):
                        st.success(get_text('analysis_applied', lang))
    ...
    if not all_versions:
        ...
        with col2:
            filter_industry = st.selectbox(
                get_text('history_filter_industry', lang),
        ...
        for idx, version in enumerate(all_versions):
            with st.expander(
                get_text('history_letter_title', lang).format(
                    ...
                    version.get('metadata', {}).get('industry', 'General')
                ),
                expanded=False
            ):
                ...
                with col2:
                    st.markdown(get_text('history_metadata', lang))
                    metadata = version.get('metadata', {})
                    st.caption(get_text('history_industry', lang).format(metadata.get('industry', 'N/A')))
                    st.caption(get_text('history_mode', lang).format(metadata.get('mode', 'N/A')))
                    st.caption(get_text('history_template', lang).format(metadata.get('template', 'N/A')))
                    ...
                    if st.button(get_text('btn_use_template', lang), key=f"use_template_{idx}"):
                        ...
                        st.success(get_text('history_loaded', lang))
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button(get_text('btn_export_all', lang)):
...
with tab4:
    ...
    with st.form("profile_form"):
        ...
        with col2:
            ...
            current_title = st.text_input(
                get_text('profile_title_field', lang),
                value=current_profile.get('current_title', ''),
                placeholder=get_text('profile_title_placeholder', lang)
            )
        ...
        if submitted:
            if not name or not email:
                st.error(get_text('profile_required_error', lang))
            else:
                profile_data = {
                    'name': name,
                    'email': email,
    ...
    st.markdown(f"### {get_text('profile_settings', lang)}")
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.checkbox(get_text('profile_notifications', lang), value=True)
        st.checkbox(get_text('profile_autosave', lang), value=True)
        st.checkbox(get_text('profile_analytics', lang), value=False)
    
    with col2:
        st.checkbox(get_text('profile_animations', lang), value=True)
        st.checkbox(get_text('profile_email_export', lang), value=False)
        st.number_input(get_text('profile_default_length', lang), min_value=200, max_value=500, value=350)
...
with tab5:
    ...
    guide_tabs = st.tabs([
        get_text('guide_quick_start', lang),
        get_text('guide_best_practices', lang),
        get_text('guide_examples', lang),
        get_text('guide_ats_tips', lang),
        get_text('guide_faq', lang)
    ])
    
    with guide_tabs[0]:
        st.markdown(f"### {get_text('guide_quick_start_title', lang)}")
        st.markdown("""
        Complete guide content here...
        """)
    
    with guide_tabs[1]:
        st.markdown(f"### {get_text('guide_best_practices_title', lang)}")
    
    with guide_tabs[2]:
        st.markdown(f"### {get_text('guide_examples_title', lang)}")
    
    with guide_tabs[3]:
        st.markdown(f"### {get_text('guide_ats_tips_title', lang)}")
